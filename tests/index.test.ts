...
describe("__offsetPlural", function () {
  it("works respecting the offset and the pluralization rules of the current locale", function () {
    let translation = (trainers) =>
      __offsetPlural(trainers, 1, {
        0: "The gym is empty",
...
describe("__select", function () {
  it("works", () => {
    expect(__select("male", { male: "He", female: "She", other: "They" })).toBe(
      "He"
    );
    expect(
      __select("female", { male: "He", female: "She", other: "They" })
    ).toBe("She");
    expect(
      __select("animal", { male: "He", female: "She", other: "They" })
    ).toBe("They");
  });
});
...
describe("__number", function () {
  ...
  it("accepts options from parsing a number skeleton as second argument", function () {
    ...
    expect(
      __number(123456789, { notation: "scientific", signDisplay: "always" })
    ).toBe("+1.235E8");
    expect(
      __number(123456789, { notation: "engineering", signDisplay: "always" })
    ).toBe("+123.457E6");
    expect(__number(123456789, { style: "unit", unit: "kilometer" })).toBe(
      "123,456,789 km"
    );
    expect(
      __number(123456789, {
...
describe("format", function () {
  it("translates the given messages on the current locale", () => {
    ...
    let unsubscribe = format.subscribe((t) => {
      ...
      expect(t("complex", { values: { a: "HA", b: "BO" } })).toBe(
        "This is a function that interpolates BO and HA"
      );
    });
    unsubscribe();
  });
